...
export function ColorTableRowActions({
  row,
  setOpenEditDialog,
  setSelectedColor,
  setData,
}: DataTableRowActionsProps<Color>) {
  const color = row.original;

  async function deleteColor() {
    try {
      const res = await apiRequest<Color>(
        `/color/delete/${color.color_id}`,
        "DELETE"
      );
      if (res.status === 200) {
        setData((prev) =>
          prev.filter((item) => item.color_id !== color.color_id)
        );
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Bạn chắc chắn muốn xóa màu sản phẩm?</DialogTitle>
          <DialogDescription>
        ...
        <DialogFooter className="sm:justify-end">
          ...
          <Button type="button" onClick={() => deleteColor()}>
            Xóa
          </Button>
        </DialogFooter>
      </DialogContent>
